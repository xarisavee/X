...
class EqualPrincipalCalculator extends Controller
{
    ...
    public function payequalPrincipalNoAjax($userdetails){
        $currentuser = $user_id;
        $coop_id = $this->getCoopId($domain);
        $userinfo = $this->getCurrentUser($domain, $user_id);
	...
	public function equalPrincipalNoAjax($userdetails){
		$currentuser = $user_id;
        $coop_id = $this->getCoopId($domain);
        $userinfo = $this->getCurrentUser($domain, $user_id);
    ...
    public function equalPrincipal(Request $request){
    ...
    public function getAmortizationSchedule($loaninstallment, $monthlyinterest, $date, $terms, $amount, $interestamt, $effectiveRate, $effectiveDuration, $effectiveRepayment){
    	$loanschedule = [];
        $amortization = [];
        $principal = [];
        $interest = [];
		...
		for($i = 0; $i < $terms; $i++){   
            if($i == 0){
                ...
                $principal[$i] = round(sprintf('%f', $amount/$effectiveRepayment), 2);
                $tempbal[$i] = $amount;
                $balance[$i] = round(sprintf('%f', $tempbal[$i]-$principal[$i]), 2);              
                $interest[$i] = $tempbal[$i]*$effectiveRate;
                $amortization[$i] = round(($principal[$i]+$interest[$i]), 2);
            }else{
                $month[$i] = $i+1;
    ...
    public function getEffectiveRepayment($repayment, $effectiveTerms, $releaseDate, $duration, $terms){
    	if($duration == "Years"){
    		...
    		$has = $terms % 12;
	    		...
	    		if(($has == 0) AND ($has_m != 0)){ // in months, whole year
		    		switch ($repayment) {
						case 'Daily':
							$startdate = strtotime($releaseDate);
