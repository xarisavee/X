...
class FlatRateCalculationController extends Controller
{
    ...
    public function savesched($domain, $user_id, $member_id, $loan_user_id){
        ...
        return $this->saveSchedDetails($domain, $coop_id, $user_id, $member_id, $loan_user_id, $schedule);
    }

    public function saveSchedDetails($domain, $coop_id, $user_id, $member_id, $loan_user_id, $schedule){
    ...
    public function payflatRateNoAjax($domain, $user_id, $member_id, $loan_user_id){
        $currentuser = $user_id;
        $coop_id = $this->getCoopId($domain);
        $userinfo = $this->getCurrentUser($domain, $user_id);
	...
	public function flatRateNoAjax($domain, $user_id, $member_id, $loan_user_id){
		$currentuser = $user_id;
        $coop_id = $this->getCoopId($domain);
        $userinfo = $this->getCurrentUser($domain, $user_id);
    ...
    public function getAmortizationSchedule($loaninstallment, $monthlyinterest, $date, $terms, $amount){
        $loanschedule = [];
        $amortization = [];
        $principal = [];
        $interest = [];
        ...
        for($i = 0; $i < $terms; $i++){   
            if($i == 0){
                ...
                $interest[$i] = round($monthlyinterest, 2);
                $principal[$i] = round(sprintf('%f', $amortization[$i]-$interest[$i]), 2);
                $balance[$i] = round(sprintf('%f', $amount), 2);
                $tempbal[$i] = round(sprintf('%f', $balance[$i]-$principal[$i]), 2);
            }else{
                $month[$i] = $i+1;
                $amortization[$i] = round($loaninstallment, 2);
                $interest[$i] = round($monthlyinterest, 2);
                $principal[$i] = round(sprintf('%f', $amortization[$i]-$interest[$i]), 2);
                $balance[$i] = round(sprintf('%f', $tempbal[$i-1]), 2);
                $tempbal[$i] = round(sprintf('%f', $balance[$i]-$principal[$i]), 2);
            }
        }
    ...
    public function getLoanInstallment($rate, $effectiveDuration, $amount, $effectiveRepayment){
        $loaninstallment = ((($amount*(($rate/100)*$effectiveDuration))+$amount) / ($effectiveRepayment));
        return $loaninstallment;
    }

    public function getInterestAmount($amount, $rate, $effectiveDuration){
    ...
    public function getEffectiveRepayment($repayment, $effectiveTerms, $releaseDate, $duration, $terms){
        if($duration == "Years"){
            ...
            $has = $terms % 12;
                ...
                else if(($has_m < 1) AND ($terms < 12)){// in months but less than a year
